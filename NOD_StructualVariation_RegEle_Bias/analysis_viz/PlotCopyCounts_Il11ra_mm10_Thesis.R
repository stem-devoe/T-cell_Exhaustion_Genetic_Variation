...
# denovo all peaks ####
b6_libSize = read.csv("/Scottbrowne/members/smd/Projects/SD032/mm10_mapped_reads.csv", 
                      header = F)
b6_libSize = b6_libSize[!grepl("mapqgt1",b6_libSize[,1]),]
b6_libSize = data.frame(SampleID = basename(b6_libSize[,1]) %>% str_remove(pattern = "_REP1.*") , 
                        libSize = b6_libSize[,2])

copy_data = read.delim("/Scottbrowne/seq/tmp/devoes/SD032/Ccl27_Ccl21_Il11ra/peaks_blast/mm10/copy_counts.tsv",
                       header = F)
...
copy_data = copy_data %>% mutate(SampleID = basename(bam) %>% str_remove(pattern = "_REP1.*")) %>% left_join(copy_data, y= metadata, by = "SampleID") %>%
  left_join(y = b6_libSize, by = "SampleID") %>% 
...
copy_data.summary = copy_data %>%
  filter(Phenotype == avg_phenotype & copy_chr == "chr4") %>% 
  group_by(copy_start) %>% 
...
ggsave(paste0("/Scottbrowne/members/smd/Projects/SD032/Ccl27_Ccl21_Il11ra/",
              Sys.Date(),"_",width,"x",height,"_",avg_phenotype,"_mm10_THESIS_Il11ra.pdf"),
       plot = p.egg,
       height = height + 4,
       width = width + 4,
       units = "in",
...
p.select = copy_data %>%
  filter(copy_chr == "chr4", peakID %in% a_peaks) %>% 
...
ggsave(paste0("/Scottbrowne/members/smd/Projects/SD032/Ccl27_Ccl21_Il11ra/",
              Sys.Date(),"_",egg.len,"in_mm10_THESIS_Il11ra-RegEle_byPeak.pdf"),
       plot = p.select.egg,
       height = 8,
       width = 10,
       units = "in",
...
# ggsave(paste0("/Scottbrowne/members/smd/Projects/SD032/Ccl27_Ccl21_Il11ra/",Sys.Date(),"_",egg.len,"in_mm10_THESIS_Il11ra-RegEle_byPeak-SetYLims.pdf"),
...
# bar graph will include non standard chromosomes in these counts which is ok but won't reflect the genome track
# would need to make sure I specify clearly or alter for this one to keep only standard chr
# this is also present for the Cd244 because two peaks had a non std hit - but I don't have the mm10 blast
# so maybe we leave as is for now
...
num_copies = num_copies %>% filter(peakID %in% plot_aes$peakID) %>% left_join(y = plot_aes, by = "peakID")
...
p.nblast = num_copies %>%
  ...
  my.theme +
  xaxis.gap.fix
p.nblast.egg = set_panel_size(p.nblast, width = unit(2,"in"), height = unit(2,"in"))
# ggsave(
#   paste0("/Scottbrowne/members/smd/Projects/SD032/Ccl27_Ccl21_Il11ra/",Sys.Date(),"_THESIS_Il11ra-RegEle_NumBlastHits_noFacet.pdf"),
#   plot = p.nblast.egg,
#   height = 6,
#   width = 6,
#   device = "pdf"
