...
motifs = lapply(motifs, FUN = function(x){y= read.delim(x, header = F, sep = "\t"); y[grepl("Common",y[,7]),]})
motifs = lapply(motif_names, FUN = function(x, motifs){motifs[[x]]$motif = x; motifs[[x]]}, motifs)
common = purrr::reduce(motifs, rbind)
write.table(common, file = "/Scottbrowne/members/smd/Projects/SD029/motif_disruption/HOCOMOCO/hocomocov11_consensus_common.bed",
            col.names = F,
            sep = "\t",
            row.names = F,
            quote = F)