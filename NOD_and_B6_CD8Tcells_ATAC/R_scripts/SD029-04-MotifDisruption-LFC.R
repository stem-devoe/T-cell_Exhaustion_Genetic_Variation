...
meta = meta[grepl(paste0(phenotype,"|Naive"),meta$Phenotype),]
...
tt.strain.exh = topTreat(tfit.strain, coef = paste0('NOD-B6_',phenotype), n = Inf)
...
# categorize phenotype comparisons
...
cate.pheno = apply(pheno.res[,c(paste0(phenotype,'-Naive.NOD'),paste0(phenotype,'-Naive.B6'))], 
                   1, 
                   FUN = categorizePeakChange)
table(cate.pheno)

# subset peaks for active and responsive element in CART compared to Naive in EITHER strain

peak_subset_gr = peaks[peaks$name %in% names(cate.pheno)[grepl("G", cate.pheno)]]
...
rtracklayer::export(peak_subset_gr,
                    paste0("/Scottbrowne/members/smd/Projects/SD029/motif_disruption/",
                           ...
                           "/Gained_",
                           phenotype,
                           "-to-Naive_any-strain_peak_subset.bed"))
...
#                             phenotype,
#                             "/",
#                             save_date, # Sys.Date part of file
#                             "_LFC_cate-motif_",motif_db,".rds"))

# assemble df for anova ####

tf.df = left_join(cate.motif, tt.strain.exh, by = "GeneID") # dont need to pre-order to match if using by in left join

# test LFC for disturbed sites ####

# run
test.spec = "B6"
#lfc.res = testLFC("Nur77", tf.df, specificity = test.spec, min_sites = 10, alt = "less")
# test all motifs
...
p.volcano = ggplot(data = lfc.res, aes(y= -log10(p.adj), x = mean_lfc, label = label)) + 
  ...
  geom_vline(xintercept = -log2(1.5), color = "red", linetype = "dashed") +
  geom_vline(xintercept = log2(1.5), color = "red", linetype = "dashed") +
